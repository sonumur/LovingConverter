...
app.post('/convert/word-to-pdf', upload.single('file'), async (req, res) => {
    ...
    try {
        ...
        log(`[W2P] Conversion proxy error: ${err.message}`);
        console.error('[W2P] Conversion proxy error:', err);
        res.status(500).json({ error: 'Conversion failed', message: err.message, debugId: 'W2P-V6' });
    } finally {
...
app.post('/convert/pdf-to-word', upload.single('file'), async (req, res) => {
    log(`Received PDF to Word request: ${req.file ? req.file.path : 'no file'}`);
    if (!req.file) {
        log('[P2W] Error: file required');
        return res.status(400).send('file required');
    }
    if (!KEY) {
        log('[P2W] Error: CloudConvert API key not configured');
        return res.status(500).send('CloudConvert API key not configured');
    }
    ...
    try {
        ...
        if (!createRes.ok) {
            const errText = await createRes.text();
            log(`[P2W] Create Job Failed: ${createRes.status} ${errText}`);
            throw new Error(`Create Job Failed: ${errText}`);
        }
        ...
        log('[P2W] Uploading file...');
        const uploadRes = await fetch(uploadUrl, { method: 'POST', body: form, headers: form.getHeaders() });
        if (!uploadRes.ok && uploadRes.status !== 204) {
            const txt = await uploadRes.text();
            log(`[P2W] Upload failed: ${txt}`);
            throw new Error('Upload failed: ' + txt);
        }
        log('[P2W] Upload successful');
        ...
        for (let i = 0; i < 120; i++) {
            ...
            jobData = await r.json();
            const status = jobData && jobData.data && jobData.data.status;
            log(`[P2W] Job status: ${status}`);

            if (status === 'finished') break;
            if (status === 'error' || status === 'failed') {
                const tasks = jobData.data.tasks || [];
                const failedTask = tasks.find(t => t.status === 'error' || t.status === 'failed');
                const taskError = failedTask ? (failedTask.message || failedTask.code) : 'Unknown task error';
                log(`[P2W] Job failed at task ${failedTask ? failedTask.operation : 'unknown'}: ${taskError}`);
                log(`[P2W] FULL JOB DATA ON ERROR: ${JSON.stringify(jobData, null, 2)}`);
                throw new Error(`Job failed: ${taskError}`);
            }
            await sleep(2000);
        }
        ...
        const fileUrl = exportTask.result.files[0].url;
        log(`[P2W] Downloading result: ${fileUrl}`);
        const fileRes = await fetch(fileUrl);
        if (!fileRes.ok) throw new Error('Download failed');
        ...
        fileRes.body.pipe(res);
        log('[P2W] Sent response');
    } catch (err) {
        log(`[P2W] Conversion proxy error: ${err.message}`);
        console.error('[P2W] Conversion proxy error:', err);
        res.status(500).json({ error: 'Conversion failed', message: err.message, debugId: 'P2W-V6' });
    } finally {
        try { fs.unlinkSync(filePath); } catch (e) { }
    }
});
